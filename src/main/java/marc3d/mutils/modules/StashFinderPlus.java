...
public class StashFinderPlus extends Module {
    ...
    private final Setting<Boolean> WebHookNotification = sgGeneral.add(new BoolSetting.Builder()
        .name("WebHookNotification")
        .description("Sends an Message to your WebHook")
        .defaultValue(false)
        .build()
    );
    ...
    public StashFinderPlus() {
        super(MUtils.CATEGORY, "StashFinder+", "Searches loaded chunks for storage blocks. Saves to <your minecraft folder>/meteor-client");
    }
    ...
    private void onChunkData(ChunkDataEvent event) {
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            ...
            if (WebHookNotification.get() && (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk))) {
                if (webhookUrl == null) {
                    ...
                    Webhook.send(webhookUrl,"Stash found at chunk " + chunk.x + " : " + chunk.z);
                }
